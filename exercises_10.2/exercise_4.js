...
na função sendMarsTemperature um outro callback que contenha as ações a serem tomadas 
em caso de falha.*/
...
const sendMarsTemperature = (callback, error) => {
  const messageSuccess = Math.random() <= 0.8;
  if (messageSuccess) return callback(getMarsTemperature());
  return error(`Robot is busy`);
}

// imprime sincronamente
